...
from helpers.main_helper import login_admin, splitter_rgb, fill_simple, waiter, poof, waiter_smart, waiter_window, \check_logs
...
def test_registration(driver):
    ...
    email = 'Kotkot_' + str(datetime.now().strftime("%Y/%m/%d@%H%M%S.qq"))
    pasw = 'qwerty'
    driver.get("http://localhost/litecart/en/create_account")
    fill_simple(driver, locator='[name="firstname"]', text="Alex")
    fill_simple(driver, locator='[name="lastname"]', text="Kot")
    fill_simple(driver, locator='[name="address1"]', text="otradnoe")
    fill_simple(driver, locator='[name="postcode"]', text="12345")
    fill_simple(driver, locator='[name="city"]', text="Moskow")
    ...
    fill_simple(driver, locator='[name="phone"]', text="+79999999999")
...
def test_buy(driver):
    ...
    #2.2) добавить его в корзину (при этом может случайно добавиться товар, который там уже есть, ничего страшного)
